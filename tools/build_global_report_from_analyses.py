...
Uso (PowerShell):
  python tools/build_global_report_from_analyses.py --analises "./import_folder/analises2" --out-csv "./import_folder/analises2/global_summary_by_column.csv" --out-html "./import_folder/analises2/relatorio_global_RANGER_SOSTAT.html" --out-db "./import_folder/analises2/global_freq_RANGER_SOSTAT.db" --table "RANGER_SOSTAT" --top-n 50 --mode slim
Uso (Bash/Zsh):
  python tools/build_global_report_from_analyses.py --analises "./import_folder/analises2" --out-csv "./import_folder/analises2/global_summary_by_column.csv" --out-html "./import_folder/analises2/relatorio_global_RANGER_SOSTAT.html" --out-db "./import_folder/analises2/global_freq_RANGER_SOSTAT.db" --table "RANGER_SOSTAT" --top-n 50 --mode slim
"""
...
def carregar_summaries(analises_dir: Path, table: str):
    ...
    for p in paths:
        ...
        for _, r in df.iterrows():
            ...
            # min/max globais
            min_val = r.get("min")
            if min_val is not None and pd.notna(min_val):
                d["mins"].append(float(min_val))
            max_val = r.get("max")
            if max_val is not None and pd.notna(max_val):
                d["maxs"].append(float(max_val))
    return acc
...
def agregar_estatisticas_numericas_precisas(acc_raw: dict):
    ...
    for col, d in acc_raw.items():
        tipo_mais = (
            max(d["tipo_counts"].items(), key=lambda kv: kv[1])[0]
            if d["tipo_counts"]
            else ""
        )
...
def gravar_db_global(df_global: pd.DataFrame, caminho_sqlite: Path):
    ...
    if not df_global.empty:
        cur.executemany(
            ...
            [
                (r["coluna"], r["valor"], int(r["count_total"]), int(r["files_count"]))
...
def construir_dataframe_global(df_global_counts: pd.DataFrame, agg_num: dict):
    ...
    cols_unicas = set(
        list(agg_num.keys())
        + (
            list(df_global_counts["coluna"].unique())
    ...
    for col in sorted(cols_unicas):
        ...
        if not df_global_counts.empty:
            ...
            if not g.empty:
                g_sorted = g.sort_values(
                    ["count_total", "files_count"], ascending=[False, False]
                )
        ...
        pct_valor_mais_freq = (
            (top1_cnt_total / nao_nulos_sum * 100.0) if nao_nulos_sum > 0 else 0.0
        )

        # distinto global = nº de valores diferentes agregados
        if not df_global_counts.empty:
            distinct_global = int(
                df_global_counts[df_global_counts["coluna"] == col]["valor"].nunique()
            )
        else:
            distinct_global = 0
    ...
    df_summary = df_summary.sort_values(
        ["nulos_pct_global", "distinct_global"],
...
def grafico_top(df_top: pd.DataFrame, coluna: str, top_n: int = 50):
    ...
    if dfv.empty:
        return "<p><em>Sem dados para gráfico.</em></p>"
    dfv = (
        dfv[["valor", "count_total", "files_count"]]
        .sort_values(["files_count", "count_total"], ascending=[False, False])
        .head(top_n)
    )
    labels = dfv["valor"].astype(str).tolist()
    counts = dfv["count_total"].astype(int).tolist()
    hover = [
        "{0}<br>contagem_total: {1}<br>apareceu_em_arquivos: {2}".format(
            html.escape(v), ct, fc
        )
        for v, ct, fc in zip(labels, counts, dfv["files_count"])
    ]
    fig = go.Figure(
        go.Bar(
            x=counts,
    ...
    fig.update_layout(
        title=f"Top valores agregados em {coluna}",
        height=max(300, 25 * len(labels)),
        margin=dict(l=250, r=30, t=40, b=30),
        xaxis_title="Contagem total (soma nos bancos)",
        yaxis_title="Valor",
    )
...
def build_table_html(summary_df: pd.DataFrame, mode: str):
    slim_cols = [
        ...
        (
            "pct_valor_mais_freq_nao_nulos_global",
    ...
    fill_colors = []
    ser = (
        summary_df["pct_valor_mais_freq_nao_nulos_global"]
        if "pct_valor_mais_freq_nao_nulos_global" in summary_df.columns
        else None
    )
...
<style>
  ...
  h2 { font-size: 14pt; line-height: 1.2; margin: 14px 0 8px; }
    ...
    html_parts.append(
        f"<h1>Relatório Global Consolidado — Tabela {html.escape(nome_tabela)}</h1>"
    )
    html_parts.append(
        "<p>Resumo por coluna com foco em Variância, Desvio Padrão e CV, mantendo a mesma organização do relatório por arquivo.</p>"
    )
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Construir relatório global (multi-banco) com mesma organização do relatório por arquivo."
    )
    p.add_argument(
        "--analises",
        required=True,
        help="Diretório base com subpastas por arquivo (cada qual contendo <TABELA>__<timestamp>/...).",
    )
    ...
    p.add_argument(
        "--table", required=True, help="Nome da tabela alvo (ex.: RANGER_SOSTAT)."
    )
    ...
    p.add_argument(
        ...
        choices=["slim", "full"],
        default="slim",
        help="Modo da tabela resumo (igual ao relatório por arquivo).",
    )
    return p.parse_args()
...
def main():
    ...
    df_global_counts, folder_ids, map_top1 = agregar_top_values(
        analises_dir, args.table
    )
